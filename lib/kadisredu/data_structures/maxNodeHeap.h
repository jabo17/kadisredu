...
inline void maxNodeHeap::reserve(NodeID capacity) {
  m_elements.reserve(capacity);
  m_element_index.reserve(capacity);
  m_heap.reserve(capacity);
};

inline maxNodeHeap::Gain maxNodeHeap::maxValue() { return m_heap[0].first; };

inline NodeID maxNodeHeap::maxElement() { return m_elements[m_heap[0].second].get_data().node; };
...
inline NodeID maxNodeHeap::size() { return m_heap.size(); }

inline bool maxNodeHeap::empty() { return m_heap.empty(); }

inline void maxNodeHeap::insert(NodeID node, Gain gain) {
  if (m_element_index.find(node) == m_element_index.end()) {
    int element_index = static_cast<int>(m_elements.size());
    int heap_size = static_cast<int>(m_heap.size());

    m_elements.emplace_back(Data(node), gain, heap_size);
    m_heap.emplace_back(gain, element_index);
...
inline void maxNodeHeap::deleteNode(NodeID node) {
  ...
  if (element_index != static_cast<int>(m_elements.size() - 1)) {
    ...
    NodeID cnode = m_elements[element_index].get_data().node;
...
inline NodeID maxNodeHeap::deleteMax() {
  if (m_heap.size() > 0) {
    ...
    NodeID node = m_elements[element_index].get_data().node;
    m_element_index.erase(node);
...
inline void maxNodeHeap::decreaseKey(NodeID node, Gain gain) {
  KASSERT(m_element_index.find(node) != m_element_index.end());
...
inline maxNodeHeap::Gain maxNodeHeap::getKey(NodeID node) {
